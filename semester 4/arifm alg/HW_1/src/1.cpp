...
//     for (auto it : g)
//     {
//         max_degree = it.size() > max_degree ? it.size() : max_degree;
//     }

//     std::vector<std::vector<double>> matrix 
//         (max_degree, std::vector<double>(max_degree + 1, 0));
    
//     for (int i = 0; i < f.size(); i++)
//     {
//         matrix[i][max_degree] = f[i];
//     }
...
// {
//     int n = a.size();        // количество уравнений (макс степень + 1)
//     int m = a[0].size() - 1; // количество неизвестных (количество многочленов g)
    
//     std::vector<int> where(m, -1); // для отслеживания главных элементов
...
//         if (fabs(a[sel][col]) < EPS) continue; // столбец нулевой, пропускаем
...
//         // Вычитаем текущую строку из всех нижележащих
//         for (int i = 0; i < n; ++i) {
...
//         }
//         // else: свободная переменная, оставляем 0
//     }
