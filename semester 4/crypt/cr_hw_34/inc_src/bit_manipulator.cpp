...
#include <stdexcept>
#include <cstdint>
#include "../include/bits/bit_manipulator.hpp"
...
bool bit_manipulator::get_bit_msb0(const std::vector<uint8_t>& data, size_t idx) const
{
    size_t byte_idx = idx / 8;
    size_t bit_in_byte = 7 - (idx % 8); // MSB0: старший бит имеет индекс 0 внутри байта
    return (data[byte_idx] >> bit_in_byte) & 1;
}

void bit_manipulator::set_bit_msb0(std::vector<uint8_t>& data, size_t idx, bool value) const
{
    size_t byte_idx = idx / 8;
    size_t bit_in_byte = 7 - (idx % 8);
    if (value)
        data[byte_idx] |= (1 << bit_in_byte);
    else
        data[byte_idx] &= ~(1 << bit_in_byte);
...
std::vector<uint8_t> bit_manipulator::extract_bits_msb0(const std::vector<uint8_t>& data,
                                                   size_t i, size_t j) const
{
    check_range(data, i, j);
    size_t len = j - i + 1;
    ...
    for (size_t pos = 0; pos < len; ++pos)
    {
        bool bit = get_bit_msb0(data, i + pos);
        if (bit)
        {
            size_t byte_idx = pos / 8;
            size_t bit_in_byte = 7 - (pos % 8); // MSB0 для результата
            result[byte_idx] |= (1 << bit_in_byte);
        }
    }
    return result;
