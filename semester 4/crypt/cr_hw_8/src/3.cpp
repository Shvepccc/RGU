...
{
    // --- Конфигурация ---
    cipher_mode_type selected_mode = cipher_mode_type::CTR;      // ECB, CBC, PCBC, CFB, OFB, CTR, RANDOM_DELTA
    padding_mode_type selected_padding = padding_mode_type::PKCS7; // Zeros, PKCS7, ISO10126, ANSI_X923
    std::size_t block_size_bits = 128; // 128, 192, 256
    std::size_t key_size_bits = 128;   // 128, 192, 256
    ...
    rijndael cipher_instance(block_size_bits);
    ...
    std::cout << "--- Synchronous Byte Array Test ---" << std::endl;
    std::vector<std::uint8_t> data_to_encrypt = {0x01, 0x02, 0x03, 0x04, 0x05, 0xAA, 0xFF};
    ...
    std::cout << "\n--- Asynchronous File Test ---" << std::endl;
    ...
    if (!check_file.good())
    {
        std::cerr << "Error: File " << input_filename << " not found." << std::endl;
        return 1;
    }
    check_file.close();
    ...
    std::string enc_file = base_name + ".enc" + extension;
    std::string dec_file = base_name + "_dec" + extension;
    ...
    try 
    {
        ...
        std::cout << "Encryption completed in " << std::chrono::duration_cast<std::chrono::milliseconds>(enc_end - enc_start).count() << " ms" << std::endl;
        ...
        std::cout << "Decryption completed in " << std::chrono::duration_cast<std::chrono::milliseconds>(dec_end - dec_start).count() << " ms" << std::endl;
        ...
        if (v1 == v2 && !v1.empty()) 
        {
            std::cout << "SUCCESS: Files are identical. Extension preserved." << std::endl;
        } 
        else 
        {
            std::cout << "FAILURE: Decrypted file differs from original or empty." << std::endl;
        }
    }
