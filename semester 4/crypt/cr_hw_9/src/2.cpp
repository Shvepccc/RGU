...
int main(int argc, char* argv[])
{
    ...
    std::size_t key_size_bits = 256;   // 128 - 448
    ...
    std::cout << "--- MARS (Multi-block Adaptive Rescoring System) Test ---" << std::endl;
    ...
    if (data_to_encrypt == decrypted_data) {
        std::cout << "Array Status: SUCCESS (Data recovered correctly)" << std::endl;
    } else {
        std::cout << "Array Status: FAILURE (Data mismatch!)" << std::endl;
    }
    ...
    if (!check_file.good()) {
        std::cerr << "Skip file test: File '" << input_filename << "' not found. "
                  << "Provide filename as argument or create 'test_data.bin'." << std::endl;
        return 0;
    }
    check_file.close();
    ...
    std::string enc_file = base_name + "_mars.enc" + extension;
    std::string dec_file = base_name + "_mars_dec" + extension;
    ...
    try {
        ...
        std::cout << "Encryption time: " << std::chrono::duration_cast<std::chrono::milliseconds>(enc_end - enc_start).count() << " ms" << std::endl;
        ...
        std::cout << "Decryption time: " << std::chrono::duration_cast<std::chrono::milliseconds>(dec_end - dec_start).count() << " ms" << std::endl;
        ...
        if (v1 == v2 && !v1.empty()) {
            ...
            std::cout << "Encrypted file saved as: " << enc_file << std::endl;
        } else {
            std::cout << "FAILURE: Decrypted file differs from original!" << std::endl;
        }
    }
    catch (const std::exception& e) {
        std::cerr << "Critical Error during file processing: " << e.what() << std::endl;
        return 1;
    }
