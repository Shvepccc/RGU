...
private:
    ...
    bigint generate_rabin_prime(std::size_t bits)
    {
...
public:
        ...
        : padding_handler(padding), B(b)
    {
        ...
        p_factor = generate_rabin_prime(key_bits / 2);
        q_factor = generate_rabin_prime(key_bits / 2);
    ...
    std::vector<uint8_t> decrypt(const std::vector<uint8_t>& encrypted_data)
    {
        ...
        bigint inv_2 = mod_inverse(2, n_public);
        ...
        for (std::size_t i = 0; i < encrypted_data.size(); i += key_bytes)
        {
            ...
            bigint sqrt2 = (n_public - sqrt1) % n_public;

            bigint sqrt3 = (sqrt_D_p * q_factor % n_public * inv_q_mod_p -
                            sqrt_D_q * p_factor % n_public * inv_p_mod_q) % n_public;
            sqrt3 = (sqrt3 + n_public) % n_public;
            bigint sqrt4 = (n_public - sqrt3) % n_public;
            ...
            for (int j = 0; j < 4 && !found; ++j)
            {
                ...
                for (int k = 0; k < 2 && !found; ++k)
                {
                    ...
                    try
                    {
                        ...
                        if (static_cast<std::size_t>(expected_len) != unpadded.size() - 2) continue;
